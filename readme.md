...
- Single Postgres-backed engine, not Zeebe's partitioned/distributed broker model. The app tier itself is stateless and already scales horizontally today — job/timer claiming uses `SKIP LOCKED`, and cross-instance gRPC job wake-ups go through Postgres `LISTEN`/`NOTIFY` rather than an in-process channel — but there's one shared Postgres primary behind it, so this scales up (bigger box, read replicas, sharding via an extension like Citus) rather than out the way Zeebe's Raft-replicated, no-shared-database partitions do. Fine for small-to-medium workloads; not the same horizontal-scaling story past a single-writer ceiling.
...
- **Cross-instance gRPC job wake-ups via Postgres `LISTEN`/`NOTIFY`** — `repository.NotifyJobAvailable`'s wake-up channel used to live purely in-process: a job created on instance A never woke an `ActivateJobs` long-poll parked on instance B, which just fell back to its 2s poll interval, silently degrading dispatch latency the moment the app tier ran as more than one instance. Fixed by moving the broadcast onto Postgres itself — `NotifyJobAvailable` now issues `SELECT pg_notify(...)`, and a dedicated `pq.Listener` connection (`StartJobWakeListener`, started once at boot) translates incoming `NOTIFY`s back into the same local wake channels, so every instance sharing the database wakes near-instantly regardless of which one created the job. This was the one concrete gap blocking the app tier from being cleanly horizontally scalable (job/timer claiming was already `SKIP LOCKED`-safe across instances) — see the [Comparison](#comparison-to-camunda-7--camunda-8) section above for what's still architecturally different from Zeebe's model.
...
- **Partitioned/distributed execution** — the app tier already scales horizontally against one Postgres (stateless instances, `SKIP LOCKED` claiming, cross-instance job wake-ups via `LISTEN`/`NOTIFY`), and Postgres itself can be pushed further with read replicas or extension-based sharding (e.g. Citus) without a rewrite. What's out of reach without one: genuinely replicating Zeebe's model — partitioning with Raft-replicated logs and no shared central database at all — which removes the single-writer ceiling entirely. That's a different engine, not an incremental change to this one — see [Comparison](#comparison-to-camunda-7--camunda-8) above. Noted here for completeness, not planned.
